...
Limitations
The User and Organization related lists are not supported as merge fields in prompt templates. The Activities related list is not supported on objects such as Account and Case. No more than 5 related list merge fields can be included.Additional Considerations Various other considerations apply to the creation of prompt templates. For instance, in case of random failures due to token limit errors, it is important to consider the dynamic nature of the number of tokens generated by a prompt template.Flex & Field Generation When using a Flex prompt template, the Add Prompt Instructions flow element can be used to incorporate dynamically generated instructions. It can be called from Apex and Lightning Web Components. A Field Generation prompt template can only be added to a Lightning record page that is upgraded to Dynamic Forms.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI1NDQ4MDE4LC0xNTM5MDMyMjAyLDgwNT
I1MDMzMCwxMDQ3NDQzODk2LDEyOTQwMjUwNSwtMTQwNDcwMzE2
OCwtNjMyOTI1NzYzLC0yNzA5MjQzNywxMTYwNzE5MjIzLC0xMT
